    ...
    function get_content(self) result(content)
      ...
      do while (associated(row_node))
        ...
        do while (associated(current_mtx_node))
          ...
          createMtxNodes = createMtxNodes // '  "' // trim(address) // '" [label ="" ,style=filled,fillcolor="'//&
          trim(current_mtx_node%value)
          write(aux2, '(I0)') current_mtx_node%col 
          createMtxNodes = createMtxNodes // '", group = "' // trim(current_mtx_node%value) // '"];' // new_line('a')
          ! align matrix nodes
